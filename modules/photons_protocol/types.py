...
from photons_protocol.fields import Optional
...
class Type(object):
    ...
    def spec(self, unpacking=False):
        ...
        * json
        """
        spec = self._spec(unpacking=unpacking)
        ...
        if self._allow_callable:
            ...
            spec = defaulted(spec, self._default)

        elif self._optional:
            spec = optional(spec)

        # if self._multiple:
        #     return multiple_spec(self, spec, unpacking)
        # else:
        return spec

    def _spec(self, unpacking=False):
        """
        Return a delfick_project.norms spec object for normalising values before and
        after packing/unpacking
        """
...
class optional(sb.Spec):
    ...
    def normalise(self, meta, val):
        if val in (sb.NotSpecified, Optional):
            return Optional
        return self.spec.normalise(meta, val)
