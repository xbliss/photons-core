...
describe "set64":
    ...
    it "can be given colors in multiple ways", seed:
        ...
        def assertColors(colors):
            expected = [Color(*c1), Color(*c2)] * 3
            cs = [
                f"{math.ceil(c.hue)},{c.saturation:0.2f},{c.brightness:0.2f},{c.kelvin}"
                for c in colors
            ]
            es = [
                f"{math.ceil(c.hue)},{c.saturation:0.2f},{c.brightness:0.2f},{c.kelvin}"
                for c in expected + [Color(0, 0, 0, 0)] * 58
            ]
        ...
        original = [
            {"hue": c[0], "saturation": c[1], "brightness": c[2], "kelvin": c[3]} for c in (c1, c2)
        ] * 3

        setting = [c1, c2, CanvasColor(*c1), CanvasColor(*c2), original[0], original[1]]
        ...
        assert real.pack() == simple.pack()
        assertColors(simple.colors)

        setting = [Color(*c1), Color(*c2)] * 3
        simple = Set64(colors=setting)
        assertColors(simple.colors)
