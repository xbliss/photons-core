        ...
        def V(self):
            class V:
                ...
                def mocked_spec(s):
                    ...
                    _spec = mock.Mock(name="_spec", return_value=spec)

                    with mock.patch.object(Type, "_spec", _spec):
                        yield spec

                    _spec.assert_called_once_with(unpacking=s.unpacking)
        ...
        it "returns default if that is specified", V:
            ...
            with V.mocked_spec() as spec:
                with mock.patch("photons_protocol.types.defaulted", defaulted):
                    assert (V.t.default(defaulter).spec(V.unpacking)) is res

                defaulted.assert_called_once_with(spec, defaulter)
            ...
            assert V.t.default(defaulter).spec().normalise(V.meta, whatever) is whatever
            assert V.t.default(defaulter).spec().normalise(V.meta, sb.NotSpecified) is val
            assert V.t.default(val).spec().normalise(V.meta, sb.NotSpecified) is val
        ...
        it "returns optional if that is specified", V:
            ...
            with V.mocked_spec() as spec:
                with mock.patch("photons_protocol.types.optional", optional):
                    assert V.t.optional().spec(V.unpacking) is res

                optional.assert_called_once_with(spec)

            # and without mocks
            whatever = mock.Mock(name="whatever")
            assert V.t.optional().spec().normalise(V.meta, whatever) is whatever
            assert V.t.optional().spec().normalise(V.meta, sb.NotSpecified) is Optional

        it "returns just the spec otherwise", V:
            with V.mocked_spec() as spec:
                assert V.t.spec(V.unpacking) is spec
    ...
    describe "hidden _spec":
        ...
        it "returns spec as is if found one", V:
            ...
            with mock.patch.object(V.t, "spec_from_conversion", spec_from_conversion):
                assert V.t._spec(unpacking=V.unpacking) is spec
        ...
        it "complains if it can't find a spec for the conversion", V:
            ...
            with assertRaises(
                ...
                with mock.patch.object(V.t, "spec_from_conversion", spec_from_conversion):
                    V.t._spec(unpacking=V.unpacking)
