...
from photons_protocol import Unset
...
scaled_hue = T.Uint16.transform(
      lambda _, v: int(65535 * (0 if v is Unset else float(v)) / 360)
    , lambda _, v: float(v) * 360 / 65535
    ).allow_float()

scaled_to_65535 = T.Uint16.transform(
      lambda _, v: int(65535 * (0 if v is Unset else float(v)))
...
waveform_skew_ratio = T.Int16.default(0).transform(
      lambda _, v: int(65535 * (0 if v is Unset else float(v))) - 32768
